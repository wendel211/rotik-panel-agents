...
**Monorepo com backend monolítico modular.** O domínio ainda é pequeno e muda de forma coordenada entre
API, banco e interface, então um repositório só reduz atrito de versionamento enquanto os módulos internos
preservam as responsabilidades. A alternativa seria separar autenticação, agentes e consumo em serviços
próprios. O custo é que os módulos compartilham processo e deploy da API: se passarem a crescer em ritmos
ou exigências operacionais diferentes, podem ser extraídos depois.

**PostgreSQL como fonte de verdade da cota.** O bloqueio depende de transação, lock e atualização
condicional, e a mesma operação precisa decidir o saldo, atualizar contadores e preservar auditoria.
Manter o contador em memória ou em um Redis separado do histórico quebraria essa unidade. O banco recebe
mais escrita e exige atenção a índices e contenção, mas a regra financeira central não fica sujeita a
consistência eventual.

**Contador consolidado no cliente.** O painel consulta consumo com custo constante, independentemente do
tamanho do histórico, em vez de recalcular com `COUNT(*)` ou `SUM()` a cada leitura. Em troca existe dado
derivado, que pode divergir se alguém escrever no banco por fora da API. Por isso a escrita fica
concentrada na transação e a divergência é acompanhada nas métricas de produção.

**SQL explícito com `pg` na operação de consumo.** O trecho mais crítico permanece visível: condição de
saldo, lock, atualização e auditoria podem ser revisados diretamente, em vez de depender do comportamento
implícito de um ORM. O preço é mais SQL manual e mapeamento de linhas, e o ganho é não esconder
concorrência e atomicidade atrás de abstração.

**Tenant obtido exclusivamente do JWT.** O cliente autenticado não consegue escolher outro tenant alterando
body, query ou parâmetros, porque a autorização nasce de uma identidade já verificada, e não de um
`clienteId` enviado pelo frontend e conferido depois. A consequência é que uma futura visão administrativa
precisará de fluxo próprio, com papel, escopo e auditoria, em vez de reaproveitar as rotas do cliente.

**Lote com consumo integral ou bloqueio integral.** Uma simulação representa uma única intenção, e aceitar
apenas parte dela produziria resultado diferente do solicitado, além de dificultar explicar consumo e
repetição. A alternativa era consumir até o saldo acabar e rejeitar o restante. Aqui o usuário precisa
reduzir o lote e reenviar, mas nunca recebe sucesso parcial silencioso.

**Paginação keyset no histórico.** O histórico cresce continuamente e precisa manter tempo de resposta
estável, inclusive durante novas inserções, o que a paginação por `OFFSET` não sustenta. O cursor é menos
flexível para saltar até uma página arbitrária, comportamento que o fluxo operacional não exige.

**React SPA com TanStack Query.** O produto é um painel autenticado, sem requisito de SEO ou renderização
pública. O TanStack Query separa estado remoto de estado visual e centraliza cache, invalidação e retries,
em vez de transformar o Context em um cache artesanal. O primeiro carregamento passa a depender do
JavaScript e da API, e em troca o deploy estático continua simples.
