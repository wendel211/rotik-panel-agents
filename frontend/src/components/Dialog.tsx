...
export function Dialog({ aberto, aoFechar, ariaLabelledby, children, className = '' }: DialogProps) {
  ...
  return (
    <dialog
      ...
      className={`m-auto max-h-[calc(100svh-2rem)] w-[calc(100%-2rem)] max-w-xl overflow-visible bg-transparent p-0 text-left text-slate-900 backdrop:bg-slate-950/55 backdrop:backdrop-blur-[2px] ${className}`}
      onClose={fechar}
      onCancel={(evento) => {
    ...
    >
      <motion.div
        className="max-h-[calc(100svh-2rem)] overflow-y-auto rounded-2xl bg-white shadow-2xl"
