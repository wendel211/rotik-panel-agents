...
export const openApiDocument = {
  ...
  paths: {
    ...
    '/agents': {
      get: {
        ...
        responses: {
          '200': { description: 'Agentes e consumo consolidado.', content: { 'application/json': { schema: object(['data'], { data: { type: 'array', items: { $ref: '#/components/schemas/Agent' } } }) } } },
          '401': unauthorized,
          '500': errorResponse('Erro interno no servidor.', 'ERRO_INTERNO'),
        },
      },
      post: {
        ...
        responses: {
          ...
          '409': errorResponse('Já existe um agente ativo com este nome.', 'AGENTE_JA_EXISTE'),
          '500': errorResponse('Erro interno no servidor.', 'ERRO_INTERNO'),
        },
      },
    },
    '/agents/{id}/executions': {
      ...
      post: {
        ...
        description: 'Consome uma unidade da cota atomicamente. No limite, registra a tentativa bloqueada e retorna 429.',
  ...
  components: {
    ...
    schemas: {
      ...
      Agent: object(['id', 'nome', 'descricao', 'status', 'bloqueado', 'criadoEm', 'ultimaExecucaoEm', 'totalExecucoes', 'plano', 'consumo'], {
        ...
        plano: object(['id', 'nome'], { id: uuid, nome: { type: 'string' } }),
      ...
      CreateExecutionInput: object([], { status: { type: 'string', enum: ['sucesso', 'erro'], default: 'sucesso' }, duracaoMs: { type: 'integer', minimum: 0, maximum: 600000 }, tokensEntrada: nonnegativeInteger, tokensSaida: nonnegativeInteger, mensagemErro: { type: 'string', maxLength: 1000, description: 'Permitida somente quando status é erro.' } }),
      CreateExecutionResponse: object(['data'], { data: object(['id', 'status', 'criadoEm', 'consumo'], { id: uuid, status: { type: 'string', enum: ['sucesso', 'erro'] }, criadoEm: dateTime, consumo: object(['usado', 'limite', 'restante'], { usado: nonnegativeInteger, limite: nonnegativeInteger, restante: nonnegativeInteger }) }) }),
      Execution: object(['id', 'status', 'duracaoMs', 'tokensEntrada', 'tokensSaida', 'mensagemErro', 'criadoEm'], { id: uuid, status: { type: 'string', enum: ['sucesso', 'erro', 'bloqueada'] }, duracaoMs: nullableInteger, tokensEntrada: nullableInteger, tokensSaida: nullableInteger, mensagemErro: nullableString, criadoEm: dateTime }),
