...
describe('shell e indicadores', () => {
  ...
  it('representa métricas normais, em risco e esgotadas', () => {
    ...
    rerender(<UtilizacaoCard usado={20} limite={100} percentual={20} totalAgentes={0} agentesAtivos={0} planoNome="Growth" />)
    expect(screen.getByText(/reinicia na virada/i)).toBeInTheDocument()
    rerender(<UtilizacaoCard usado={85} limite={100} percentual={85} totalAgentes={4} agentesAtivos={3} planoNome="Growth" />)
    expect(screen.getByText(/restam/i)).toBeInTheDocument()
    rerender(<UtilizacaoCard usado={100} limite={100} percentual={100} totalAgentes={4} agentesAtivos={3} planoNome="Growth" />)
...
describe('agentes e limite', () => {
  it('renderiza os estados e aciona simulação/histórico', async () => {
    ...
    rerender(<AgentRow agente={agente({ bloqueado: true })} indice={0} simulando={false} aoSimular={simular} aoAbrirHistorico={historico} />)
